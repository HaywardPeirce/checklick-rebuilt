...
if($username&&$password){
	...
	mysql_select_db("checkclick") or die("couldnt find DB");
	//query the server getting all users login info
	$query= mysql_query("SELECT * FROM instructor WHERE username='$username'");
	...
	if($numrows!=0){
		...
		if($username==$dbusername&&md5($password)==$dbpassword){
			...
			//get everything from the instructor table in the column with the username $username
			$get = mysql_query("SELECT * FROM `instructor` WHERE username ='$username'");
			$_SESSION['name'] = $name; 
			if($row = mysql_fetch_assoc($get)){
				...
				$instructor_session = $row['current_session'];
				$_SESSION['instructor_session'] = $instructor_session;
